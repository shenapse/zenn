...
もちろん, 特別扱いするに値するノルム, たとえばソボレフ空間のノルムなどは, `\norm` コマンドの特殊化として `\normSobolev{f}[p=p, order=k, space=W]` などと定義してよい.
