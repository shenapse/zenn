...
数理系の文書を想定し, 私が思う $\LaTeX$ コードの可読性・保守性を上げる方法を紹介する.
一般論を扱おうとすると内容が膨大になってしまうので, この記事では数式コマンドのデザインだけに焦点を当てる.
この点に絞る理由は以下の通り:
...
説明不要なほど当然のルールだが, 念のため. コマンドには読み手がその機能を想像でき, かつ他のコマンドと容易に区別できる示唆的な名前を付けるべきだ. 過度な略称は控えた方がよい.
...
なお, 当然だが, 読み上げ順は何らかの対応概念の存在を前提としている. 蛇足だが, このような形で読み上げることができないコマンドは, 何らかの改善の余地がある場合が多い。
...
以下は重要度順等ではなく, 説明する上で都合がよい順.

### 繰り返し登場する概念があるとき → する

対応概念が有り, かつそれなりの回数登場する記号は, コマンド化した方がよい.
コマンド化することで可読性も増すだろうし, その概念に当てる記号をコマンドという単位で管理できる. 結果的に保守性も上がる.

### 1つのコマンドが異なる意味で使われているとき → する

典型的なケースとしては, 用途の広いパッケージの関数を `document` 環境にベタ書きしているケースだ.
...
### 1つの記号の用途が2つ以上になったとき → する

記号の用途が重なるなら少なくとも片方(対応概念がある方)は, 現時点の出現回数を問わずコマンド化した方がよい.
記号変更したくなったときに, `document` 環境内におけるその記号の用途が複数あると, 置換作業の手間が劇的に増えてしまう.

たとえば, 集合 $A$ の内部を `A^{\circ}`, 関数の合成を `g \circ f` と書いているなら, 可読性への寄与を考慮して, 前者を `\interior{A}` として定義した方がよいだろう.
もちろん, 同時に後者を `g \composite f` などとコマンド化してもよい.

### 合成コマンドが明確な対応概念を持つとき → する

コマンドの合成の乱用は依存関係を複雑にして, 保守性を下げる場合があるので慎重になるべきということはまず明言しておく.
しかし, このような合成の需要は, 可読性の向上を目的として多々発生する.
...
```tex
\tanBasisTrans{from=x_{i_1}, to={\~{x}}, subscript={j_1}}[p]
```

のようなコマンドを定義すると可読性が上がり, 意味も明確になる. その実装は, 既存のコマンドを併記するような内容になるので, 直観的でメンテナンスが楽な内容に収まりやすい.
